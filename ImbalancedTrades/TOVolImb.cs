...
namespace ATAS.Indicators.Technical
{
    ...
    public class TOVolImb : Indicator
    {
        ...
        protected override void OnCalculate(int bar, decimal value)
        {
            ...
            var highPen = new Pen(new SolidBrush(Color.FromArgb(255, 175, 203, 255)))
            ...
            foreach (LineTillTouch ltt in HorizontalLinesTillTouch)
            {
                ...
                if (ltt.Finished && ltt.SecondBar == bar && iDistance > iMinBars)
                {
                    ...
                    if ((cc.High >= ltt.SecondPrice &&
                        cc.Close < ltt.SecondPrice &&
                        cc.Open < ltt.SecondPrice)
                      || (cc.Low <= ltt.SecondPrice &&
                      cc.Close > ltt.SecondPrice &&
                      cc.Open > ltt.SecondPrice))
                    {
                        // WICK
                        _paintBars[bar] = MColor.FromRgb(229, 250, 42); // yellow
                    }
                    else
                    {
                        // FULL TOUCH
                        _paintBars[bar] = MColor.FromRgb(133, 195, 242); // cyan
                    }
                    break;
                }
